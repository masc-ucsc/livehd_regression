...
void functionGen(FILE *v, FILE *c, FILE *p, int inputs, int bit_max, int width, int budget, bool _signed, bool force_concat, bool allow_invert, bool no_mult, bool force_constants, bool allow_constants, int limit, bool has_child, bool entropy){
	...
	// PYROPE-REMOVE
	if(1/*xorshift32()%3*/)
		binaryGen(v, c, p, inputs, bit_max, width, budget, _signed, invert, force_concat, no_mult, force_constants, allow_constants, limit, has_child, entropy);		
	else{	
		if(_signed){
			fprintf(v, "$signed(" );
		}
		...
		concatGen(v, c, p, inputs, bit_max, width, budget, force_concat, no_mult, force_constants, allow_constants, limit, has_child, entropy);	//concat always returns uints so can be simple recasted
		if(_signed){
			fprintf(v, ")" );
			fprintf(c, ".asSInt");
		}
...
void concatGen(FILE *v, FILE *c, FILE *p, int inputs, uint32_t bit_max, uint32_t width, int budget, bool force_concat, bool no_mult, bool force_constants, bool allow_constants, int limit, bool has_child, bool entropy){
	budget--;
	fprintf(v, "{");
	fprintf(c, "Cat(");

	if(!budget)
		randomInput(v, c, p, inputs, bit_max, width, 0, 1, force_constants, allow_constants, limit, has_child, entropy);
	else {
		while(width > 0) {
			...
			if(one_out_of(3, true, true)/*xorshift32()%3*/) {
				binaryGen(v, c, p, inputs, bit_max, sub_width, budget, 0, 1, force_concat, no_mult, force_constants, allow_constants, limit, has_child, entropy);
			}
			else {
				concatGen(v, c, p, inputs, bit_max, sub_width, sub_width==1 ? 1 : budget, force_concat, no_mult, force_constants, allow_constants, limit, has_child, entropy);		//cant split anymore once sub_width is 1
			}
	...
	//else{	//otherwise cuts off by a single bit
		...
		*/
		...
		if(budget==0){
			...
			randomInput(v, c, p, inputs, bit_max, width,_signed, allow_invert, force_constants, allow_constants, limit, has_child, entropy);
		}
		else{	
			...
			if(one_out_of(2, op > 7 || op < 3)/*xorshift32()%(1 + (op>7 || op<3))*/) {	//expands, prevents nested arithmetic operations, those become too confusing for yosys compiler
				functionGen(v, c, p, inputs, bit_max, width_1, budget, shift_sign, 0, allow_invert, no_mult, force_constants, allow_constants, limit, has_child, entropy);
			}
			else{
				//binary operation
				randomInput(v, c, p, inputs, bit_max, width_1, shift_sign, allow_invert, force_constants, allow_constants, limit, has_child, entropy);
			}
			...
			if(one_out_of(2, op > 7 || op < 3)/*xorshift32()%(1 + (op>7 || op<3))*/) {	//expands
				functionGen(v, c, p, inputs, bit_max, width_2, budget, op>7?0:_signed, 0, allow_invert, no_mult, force_constants, allow_constants, limit, has_child, entropy); 	//prevents shifting by a negative number, forces uint to be used
			}
			else{
				//binary operation
				randomInput(v, c, p, inputs, bit_max, width_2, op>7?0:_signed, allow_invert, force_constants, allow_constants, limit, has_child, entropy);	//prevents shifting by a negative number, forces uint to be used
			}
...
void randomInput(FILE *v, FILE *c, FILE *p, int inputs, int bit_max, int width, bool _signed, bool allow_invert, bool force_constants, bool allow_constants, int limit, bool has_child, bool entropy){
	...
	bool use_constant = one_out_of(CHANCE_OF_CONSTANT, allow_constants, true);	//determines if a constant or an input will be returned
...
void randomConstant(FILE *v, FILE *c, int width, bool _signed) {
	bool neg = one_out_of(2, _signed);	//rolls whether negative or not
	int value = num_less_than((1 << width)/(1+_signed));	//2's complement range stunted by 1/2
...
void printAssignments(FILE *v, FILE *c, FILE *p, int inputs, int bit_max, int width, int budget, int wire_number, bool _signed, bool force_constants, bool allow_constants, bool has_child, bool entropy) {
	...
	if(one_out_of(2)){		//No Conditionals
		fprintf(c, "\n	%c%d := ",z,wire_number);
		fprintf(p, "\n%c%d = ",z,wire_number);
		functionGen(v, c, p, inputs, bit_max, width, budget, width%2, 0, 1, 0, force_constants, allow_constants, wire_number, has_child, entropy);
	}
		...
		if(one_out_of(2, budget)) {
			functionGen(v,c,p,inputs,bit_max,1,budget,0,0, 1,0,force_constants,allow_constants,wire_number,has_child, entropy);
		}
		else{
			// Request single bit from randomInput
			randomInput(v,c,p,inputs,bit_max,1,0, 1,force_constants,allow_constants,wire_number,has_child,entropy);
		}
		...
		if(one_out_of(2, budget)) {
			functionGen(v,c,p,inputs,bit_max,width,budget,_signed,0, 1,0,force_constants,allow_constants,wire_number,has_child, entropy);
		}
		else{
			//print ternary opearnd
			randomInput(v,c,p,inputs,bit_max,width,_signed, 1,force_constants,allow_constants,wire_number,has_child,entropy);
		}
		...
		if(one_out_of(2, budget)) {
			functionGen(v,c,p,inputs,bit_max,width,budget,_signed,!_signed, 1,0,force_constants,allow_constants,wire_number,has_child, entropy);	//forces concatenation for signed right shift to preserve signness
		}
		else{
			//print ternary opearnd
			randomInput(v,c,p,inputs,bit_max,width,_signed, 1,force_constants,allow_constants,wire_number,has_child,entropy);
